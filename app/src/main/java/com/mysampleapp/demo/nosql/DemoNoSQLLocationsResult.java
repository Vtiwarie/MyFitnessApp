...
public class DemoNoSQLLocationsResult implements DemoNoSQLResult {
    ...
    public void updateItem() {
        ...
        final double originalValue = result.getDistance();
        result.setDistance(DemoSampleDataGenerator.getRandomSampleNumber());
        try {
            mapper.save(result);
        } catch (final AmazonClientException ex) {
            // Restore original data if save fails, and re-throw.
            result.setDistance(originalValue);
            throw ex;
        }
    }
    ...
    public View getView(final Context context, final View convertView, int position) {
        ...
        final TextView distanceKeyTextView;
        final TextView distanceValueTextView;
        if (convertView == null) {
            ...
            distanceKeyTextView = new TextView(context);
            distanceValueTextView = new TextView(context);
            setKeyAndValueTextViewStyles(distanceKeyTextView, distanceValueTextView);
            layout.addView(distanceKeyTextView);
            layout.addView(distanceValueTextView);
        } else {
            ...
            distanceKeyTextView = (TextView) layout.getChildAt(5);
            distanceValueTextView = (TextView) layout.getChildAt(6);
        }
        ...
        distanceKeyTextView.setText("distance");
        distanceValueTextView.setText("" + result.getDistance().longValue());
        return layout;
    }
}
